...
class Dataset(data.Dataset):
    ...
    def load_mel_data(self, mel_data, mode, path_name="", index=None):
        ...
        if mode == "training":
            ...
            raise ValueError(
                "Dataset, load_mel_data: mode must be set to training, validatation, or testing."
            )
    ...
    def __getitem__(self, index):
        ...
        self.add_images(
            X, 0, path_name, run_ids[0], pose_ids[0], self.use_normalization
        )  # Source image
        self.add_images(
            X, 1, path_name, run_ids[1], pose_ids[1], self.use_normalization
        )  # Target image

        if self.use_disparity:
            self.add_disparity(
                D, 0, path_name, run_ids[0], pose_ids[0]
            )  # Source disparity
            self.add_disparity(
                D, 1, path_name, run_ids[1], pose_ids[1]
            )  # Target disparity
    ...
    def add_image(self, X, start_ind, loc, path_name, run_id, pose_id, normalize_img):
        ...
        img_file = (
            f"{self.data_dir}/{path_name}/run_{run_id.zfill(6)}/"
    ...
    def add_disparity(self, D, index, path_name, run_id, pose_id):
        ...
        img_file_left = (
            f"{self.data_dir}/{path_name}/run_{run_id.zfill(6)}/images/"
