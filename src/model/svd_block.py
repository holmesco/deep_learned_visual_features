...
class SVDBlock(nn.Module):
    ...
    def forward(self, keypoints_3D_src, keypoints_3D_trg, weights):
        ...
        centroid_src = torch.sum(
            keypoints_3D_src[:, 0:3, :] * weights, dim=2, keepdim=True
        ) / torch.sum(
            weights, dim=2, keepdim=True
        )  # Bx3x1
        centroid_trg = torch.sum(
            keypoints_3D_trg[:, 0:3, :] * weights, dim=2, keepdim=True
        ) / torch.sum(weights, dim=2, keepdim=True)
        ...
        H = (1.0 / w) * torch.bmm(
            trg_centered, torch.bmm(W, src_centered.transpose(2, 1).contiguous())
        )  # Bx3x3
        ...
        R_trg_src = torch.bmm(
            U, torch.bmm(diag, V.transpose(2, 1).contiguous())
        )  # Bx3x3
        t_trg_src_insrc = centroid_src - torch.bmm(
            R_trg_src.transpose(2, 1).contiguous(), centroid_trg
        )  # Bx3x1
        t_src_trg_intrg = -R_trg_src.bmm(
            t_trg_src_insrc
