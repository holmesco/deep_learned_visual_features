...
class MatcherBlock(nn.Module):
    ...
    def forward(self, kpt_2D_src, kpt_2D_trg, kpt_desc_norm_src, kpt_desc_norm_trg):
        ...
        match_vals = torch.matmul(
            kpt_desc_norm_src.transpose(2, 1), kpt_desc_norm_trg
        ) / float(
            kpt_desc_norm_trg.size(1)
        )  # BxNx(HW or N)
        ...
        kpt_2D_pseudo = torch.matmul(
            kpt_2D_trg.to(soft_match_vals), soft_match_vals.transpose(2, 1)
        )
