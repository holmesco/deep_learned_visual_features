...
class LieOptBlock(nn.Module):
    ...
    def __init__(self, T_s_v, N_batch, N_map, opt_kwargs_in={}):
        ...
        Args:
            T_s_v (torch.tensor): 4x4 transformation matrix providing the transform from the vehicle frame to the
                                  sensor frame.
        """
        super(LieOptBlock, self).__init__()
        ...
        aux_vars = [keypoints_3D_trg, keypoints_3D_src, weights, inv_cov_weights]
    ...
    def error_fn_vec(optim_vars, aux_vars):
        ...
        keypoints_3D_trg, keypoints_3D_src, weights, inv_cov_weights = aux_vars
        # Get dimensions
        ...
        errors = keypoints_3D_trg.tensor - T_trg_src_mat.bmm(keypoints_3D_src.tensor)
        ...
        if inv_cov_weights is not None:
            ...
            errors_weighted = torch.einsum("bnij,bjn->bin", W_half, errors[:, :3, :])
            errors_weighted = errors_weighted * weights.tensor
        else:
            errors_weighted = errors[:, :3, :] * weights.tensor
    ...
    def forward(
        ...
        if inv_cov_weights is None:
            inv_cov_weights = torch.eye(3).expand(batch_size, self.N_map, 3, 3).cuda()
