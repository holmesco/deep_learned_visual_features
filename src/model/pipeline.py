...
class Pipeline(nn.Module):
    ...
    def forward(self, net, images, disparities, pose_se3, pose_log, epoch, test=False):
        ...
        ################################################################################################################
        with record_function("Keypoint detection"):
        ...
        with record_function("Get 2D Keypoints"):
        ...
        with record_function("Get 3D Keypoints"):
            ...
            kpt_3D_trg, kpt_valid_trg, kpt_desc_norm_trg, kpt_scores_trg = (
                get_keypoint_info(
                    kpt_2D_trg,
                    scores_trg,
                    descriptors_trg,
        ...
        ################################################################################################################
        with record_function("Keypoint matching"):
            if self.config["pipeline"]["dense_matching"]:
                ...
                kpt_2D_trg_dense = self.image_coords.unsqueeze(0).expand(
                    batch_size, 2, self.height * self.width
                )

                # Compute the coordinates of the matched keypoints in the target frame, which we refer to as pseudo points.
                kpt_2D_pseudo = self.matcher_block(
                    kpt_2D_src, kpt_2D_trg_dense, kpt_desc_norm_src, desc_norm_trg_dense
                )
                ...
                kpt_2D_pseudo = self.matcher_block(
                    kpt_2D_src, kpt_2D_trg, kpt_desc_norm_src, kpt_desc_norm_trg
                )
            ...
            kpt_3D_pseudo, kpt_valid_pseudo, kpt_desc_norm_pseudo, kpt_scores_pseudo = (
                get_keypoint_info(
                    kpt_2D_pseudo,
                    scores_trg,
                    descriptors_trg,
        ...
        with record_function("Outlier rejection"):
            ...
            if self.config["outlier_rejection"]["on"] and (
                ...
                T_trg_src_gt_inl = (
                    pose_se3  # Ground truth transform to use to find inliers.
                )
                ...
                if "plane" in self.config["outlier_rejection"]["dim"]:
                    ...
                    rot_col1 = torch.stack(
                        [
                            torch.cos(log_pose[:, 5]),
                            torch.sin(log_pose[:, 5]),
                            zero.clone(),
                            zero.clone(),
                        ],
                        dim=1,
                    )  # Bx4
                    rot_col2 = torch.stack(
                        [
                            -torch.sin(log_pose[:, 5]),
                            torch.cos(log_pose[:, 5]),
                            zero.clone(),
                    ...
                    rot_col3 = torch.stack(
                        [zero.clone(), zero.clone(), one.clone(), zero.clone()], dim=1
                    )  # Bx4
                    trans_col = torch.stack(
                        [log_pose[:, 0], log_pose[:, 1], zero.clone(), one.clone()],
                        dim=1,
                    )  # Bx4

                    T_trg_src_gt_inl = torch.stack(
                        [rot_col1, rot_col2, rot_col3, trans_col], dim=2
                    )  # Bx4x4

                T_s_v = self.T_s_v.expand(batch_size, 4, 4)
                T_trg_src_gt_inl_sensor = T_s_v.bmm(T_trg_src_gt_inl).bmm(
                    se3_inv(T_s_v)
                ...
                kpt_2D_pseudo_gt = self.stereo_cam.camera_model(kpt_3D_pseudo_gt)[
                    :, 0:2, :
                ]  # Bx2xN
                ...
                if "3D" in self.config["outlier_rejection"]["dim"]:
                    ...
                    inliers_3D = (
                        err < self.config["outlier_rejection"]["error_tolerance"]["3D"]
                    )
                ...
                if "2D" in self.config["outlier_rejection"]["dim"]:
                    ...
                    inliers_2D = (
                        err < self.config["outlier_rejection"]["error_tolerance"]["2D"]
                    )
                    valid_inliers = valid_inliers & (inliers_2D.unsqueeze(1))

                if "plane" in self.config["outlier_rejection"]["dim"]:
                    # Do comparison in the vehicle frame, not the sensor frame.
                    kpt_3D_pseudo_gt_vehicle = T_trg_src_gt_inl.bmm(
                        se3_inv(T_s_v).bmm(kpt_3D_src)
                    )  # Bx4xN
                    kpt_3D_pseudo_vehicle = se3_inv(T_s_v).bmm(kpt_3D_pseudo)
                    err = torch.norm(
                        kpt_3D_pseudo_vehicle[:, 0:2, :]
                        - kpt_3D_pseudo_gt_vehicle[:, 0:2, :],
                        dim=1,
                    )  # BxN
                    inliers_plane = (
                        err
                        < self.config["outlier_rejection"]["error_tolerance"]["plane"]
                    )
        ...
        with record_function("Loss computation"):
            ...
            if "keypoint_3D" in loss_types:
                ...
                keypoint_loss = mse_loss_fn(
                    kpt_3D_pseudo[valid], kpt_3D_pseudo_gt[valid]
                )
            ...
            if "keypoint_2D" in loss_types:
                ...
                keypoint_loss = mse_loss_fn(
                    kpt_2D_pseudo[valid], kpt_2D_pseudo_gt[valid]
                )
            ...
            if "keypoint_plane" in loss_types:
                ...
                kpt_3D_pseudo_vehicle = se3_inv(T_s_v).bmm(kpt_3D_pseudo)
                keypoint_loss = mse_loss_fn(
                    kpt_3D_pseudo_vehicle[:, 0:2, :][valid],
            ...
            if test or (epoch >= self.config["training"]["start_pose_estimation"]):
                if "pose" in loss_types:
                    ...
                    rot_loss, trans_loss = self.pose_loss(
                        T_trg_src_gt, T_trg_src, mse_loss_fn
                    )
                    ...
                    pose_loss = (1.0 / batch_size) * torch.sum(
                        log_pose_err.transpose(2, 1).bmm(w).bmm(log_pose_err)
                    )
