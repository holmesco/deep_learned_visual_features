...
class Pipeline(nn.Module):
    ...
    def forward(self, net, images, disparities, pose_se3, pose_log, epoch, test=False):
        ...
        weights = self.weight_block(
            kpt_desc_norm_src, kpt_desc_norm_pseudo, kpt_scores_src, kpt_scores_pseudo
        )
        ...
        if self.config["outlier_rejection"]["on"] and (
            ...
            if "plane" in self.config["outlier_rejection"]["dim"]:
                ...
                trans_col = torch.stack(
                    [log_pose[:, 0], log_pose[:, 1], zero.clone(), one.clone()], dim=1
                )  # Bx4
            ...
            if "plane" in self.config["outlier_rejection"]["dim"]:
                ...
                inliers_plane = (
                    err < self.config["outlier_rejection"]["error_tolerance"]["plane"]
                )
