...
class StereoCameraModel(nn.Module):
    ...
    def check_valid_disparity(self, disparity):
        ...
        """
        disp_min = (self.f * self.b) / 100.0  # Farthest point 100 m
    ...
    def image_to_camera(self, img_coords, disparity, Q):
        ...
        img_coords_norm = self.normalize_coords(
            img_coords, batch_size, height, width
        ).unsqueeze(
            1
        )  # Bx1xNx2

        point_disparities = F.grid_sample(
            disparity, img_coords_norm, mode="nearest", padding_mode="border"
        )  # Bx1x1xN
