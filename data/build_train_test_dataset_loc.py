...
def main(config):
    ...
    for path_name in config["train_paths"]:
        num_samples_train[path_name] = math.floor(
            config["sampling_ratios_train"][path_name] * config["num_train_samples"]
        )

    num_samples_valid = {}
    for path_name in config["test_paths"]:
        num_samples_valid[path_name] = math.floor(
            config["sampling_ratios_valid"][path_name]
            * config["num_validation_samples"]
        )

    # Training and validation data
    train_ids, train_labels_se3, train_labels_log = build_random_loc_dataset(
        data_path,
        config["train_paths"],
        config["train_runs"],
        num_samples_train,
    ...
    valid_ids, valid_labels_se3, valid_labels_log = build_random_loc_dataset(
        data_path,
        config["test_paths"],
    ...
    mean, std_dev = compute_mean_std(
        mel_data, data_path, config["dataset"]["height"], config["dataset"]["width"]
    )
    mel_data.mean = mean
    ...
    for path_name in config["test_paths"]:
        ...
        for i in range(len(test_runs) - 1):
            ...
            mel_data.test_labels_se3[path_name] = mel_data.test_labels_se3[
                path_name
            ] + [test_labels_se3]
            mel_data.test_labels_log[path_name] = mel_data.test_labels_log[
                path_name
            ] + [test_labels_log]
    ...
    print(
        f"Generating full training, validation, and test dataset took {time.time() - start} s"
    )
