...
from data.sample_data import random_sample, sequential_sample, single_sample
...
def build_random_loc_dataset(data_path, path_names, runs, num_samples, temporal_length):
    ...
    for path_name in path_names:
        ...
        path_ids, path_labels_se3, path_labels_log = random_sample(
            path_name, pose_graph, runs[path_name], num_samples_path, temporal_length
        )
...
def build_dataset_from_ids(data_path, sample_ids):
    """Build dataset based on provided sample ids

    Args:
        data_path (str): path to the data directory
        sample_id (str): string of the form pathname_runid1_poseid1_runid2_poseid2 specifying the sample
    ...
    for sample_id in sample_ids:
        ...
        live_run_id = int(live_run_id)

        # Build pose graph
        data_dir = f"{data_path}/{path_name}"
        pose_graph = build_sub_graph([map_run_id, live_run_id], data_dir)
        # Get sample data
        label_se3, label_log = single_sample(
            pose_graph, map_run_id, map_frm_id, live_run_id, live_frm_id
        )
        labels_log[sample_id] = label_log
        labels_se3[sample_id] = label_se3

    return labels_se3, labels_log
