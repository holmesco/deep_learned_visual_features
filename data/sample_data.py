...
def random_sample(path_name, pose_graph, runs, num_samples, max_temporal_length):
    ...
    while sample_counter < num_samples:
        ...
        try:
            neighbour_ids = pose_graph.get_topo_neighbours(
                live_id, max_temporal_length + 1
            )
        ...
        if len(neighbour_ids) == 0:
            print(
                f"{path_name} - Random sampling: Could not find neighbours for vertex {live_id} within topological"
        ...
        if other_id[0] not in runs:
            print(
                f"{path_name} - Random sampling: other vertex is in run {other_id[0]}, which is not in desired runs"
            )
            continue

        if live_id[0] == other_id[0]:
            print(
                f"{path_name} - Random sampling: do not want to localize to vertex on the same run"
            )
            continue
        ...
        if not sample_id in samples:
            ...
            print(
                f"{path_name} - Random sampling: sample {sample_id} has already been added",
                flush=True,
            )
        ...
        if (live_id[0] in live_runs) and (live_id[0] != map_run_id):
            ...
            while (map_pose_id == -1) and (radius <= max_radius):
                try:
                    ...
                    print(e)
                    print(
                        f"{path_name} - Sequential sampling: Could not localize {live_id} to map run, topological"
                        f" neighbours failed"
                    )
                    radius += 1
                    continue
            ...
            if map_pose_id == -1:
                print(
                    f"{path_name} - Sequential sampling: Could not localize {live_id} to map run "
            ...
            if chosen_topo_dist != temporal_length + 1:
                print(
                    f"{path_name} - Sequential sampling: Could not match {live_id} at topological distance "
            ...
            if map_vertex is None:
                print(
                    f"{path_name} - Sequential sampling: Could not localize {live_id} at topological distance "
            ...
            if not sample_id in samples:
                ...
                samples.append(sample_id)
                labels_se3[sample_id] = torch.tensor(
                    T_map_live.matrix, dtype=torch.float
                )
                ...
                print(
                    f"{path_name} - Sequential sampling: sample {sample_id} has already been added"
                )

    return samples, labels_se3, labels_log


def single_sample(pose_graph, map_run_id, map_frm_id, live_run_id, live_frm_id):
    """
    Retrieve a single sample (relative pose transform) based on the sample id.
    T_v2_v1 is on the form pathname_runid1_poseid1_runid2_poseid2, for instance: mutliseason_1_531_5_542.

    Args:
        pose_graph (Graph): the pose graph.
        map_run_id (int): id of the map run.
        map_frm_id (int): id of the map frame.
        live_run_id (int): id of the live run.
        live_frm_id (int): id of the live frame.
    ...
    # Get the map vertex we want to localize to.
    map_id = (map_run_id, map_frm_id)
    live_id = (live_run_id, live_frm_id)

    # Get relative transform
    T_map_live = pose_graph.get_transform(live_id, map_id)
    log_map_live = Transform.LogMap(T_map_live)
    # Convert to torch tensor.
    label_se3 = torch.tensor(T_map_live.matrix, dtype=torch.float)
    label_log = torch.tensor(log_map_live, dtype=torch.float)

    return label_se3, label_log
